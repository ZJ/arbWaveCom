...
int parseOptions(
	...
	case 'i':
	    ...
	    if (NULL != options->inputPath)
		free(options->inputPath);
	...
	if (g_opt_debug)
	    printf("Found \"%c\" (or equiv.) with argument %s\n", currentOption, optarg);
	...
	if (NULL != templateFile) {
	    ...
	    fclose(templateFile);
	    if (!g_opt_quiet)
		printf("Template file written to " TEMPLATE_FILENAME ".\n");
	} else {
	    perror("Writing template file");
	    return OPT_RET_ERR;
	}
    ...
    if ((options->flags & OPT_FROMCMD_MASK)
	&& !((options->flags & OPT_ALLSET_MASK) == OPT_ALLSET_MASK)) {
